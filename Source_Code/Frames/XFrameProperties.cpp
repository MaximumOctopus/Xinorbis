...
void TFrameProperties::BuildMagnitudeChart(bool range)
{
	...
	vtcMagnitude->Title->Text->Strings[0] = GScanEngine->Path.String.c_str();
	...
	for (int t = 0; t < __MagnitudesCount; t++)
	{
		if (data_range == 0)
		{
			if (GScanEngine->Data.Magnitude[t].Count != 0)
			{
				vtcMagnitude->SeriesList->Items[0]->Add(GScanEngine->Data.Magnitude[t].Count, __MagnitudeLabelsShort[t].c_str(), TColor(__MagnitudeColours[t]));
			}
		}
		else
		{
			if (GScanEngine->Data.Magnitude[t].Size != 0)
			{
				vtcMagnitude->SeriesList->Items[0]->Add(GScanEngine->Data.Magnitude[t].Size, __MagnitudeLabelsShort[t].c_str(), TColor(__MagnitudeColours[t]));
			}
		}
	}
}
