...
begin
  ...
  lThings[1] := XText[kOn];
  lThings[2] := XText[kBefore];
  lThings[3] := XText[kAfter];
  ...
  sgExplore.Cells[colFolderName,     0] := XText[kFolder];
  sgExplore.Cells[colFileCount,      0] := '# ' + XText[kFiles];
  sgExplore.Cells[colFilesAsPercent, 0] := XText[kAsPercent];
  sgExplore.Cells[colSize,           0] := XText[kSize];
  sgExplore.Cells[colSizeAsPercent,  0] := XText[kAsPercent];
  ...
  miExploreFolder.Caption := XText[kExploreFolder];

  // ===========================================================================

  cbSLTUnit.Items.Add(XText[kXBytes]);
  cbSLTUnit.Items.Add(XText[kXKilobytes]);
  cbSLTUnit.Items.Add(XText[kXMegabytes]);
  cbSLTUnit.Items.Add(XText[kXGigabytes]);
  cbSLTUnit.ItemIndex := 0;

  cbSGTUnit.Items.Add(XText[kXBytes]);
  cbSGTUnit.Items.Add(XText[kXKilobytes]);
  cbSGTUnit.Items.Add(XText[kXMegabytes]);
  cbSGTUnit.Items.Add(XText[kXGigabytes]);
  cbSGTUnit.ItemIndex := 0;

  // ===========================================================================

  bHelp.Caption        := XText[kHelp];
  bClose.Caption       := XText[kClose];

  cbContaining.Caption := XText[kContainingText];
  cbSize.Caption       := XText[kFileSize];
  cbCategory.Caption   := XText[kCategory];
  cbAttributes.Caption := XText[kFileAttributes];
  cbCreated.Caption    := XText[kCreatedDate];
  cbAccessed.Caption   := XText[kAccessedDate];
  cbModified.Caption   := XText[kModifiedDate];
  cbOwner.Caption      := XText[kFileOwner];
  cbNameLength.Caption := XText[kFileNameLength];

  lContaining.Caption  := XText[kContains];
  ...
  cbATemp.Caption      := XText[kTemporary];

  bFilter.Caption      := XText[kFilter];
  ...
  cbUSelect.Items.Add(XText[kEqualTo]);
  cbUSelect.Items.Add(XText[kNotEqualTo]);
  cbUSelect.Items.Add(XText[kContains]);
  cbUSelect.Items.Add(XText[kDoesntContain]);
...
procedure TfrmExplore.BuildFrom(const NewScanPath : string);
 ...
 begin
  ...
  Caption := XText[kExploring] + ': ' + NewScanPath;
  ...
  lData.HTMLText[0] := '<b>' + IntToStr(TotalFCount) + '</b> ' + XText[kFiles] + '; <b>' + IntToStr(TotalDCount) + '</b> ' + XText[kFolders] + '; <b>' + TConvert.ConvertToUsefulUnit(TotalSize) + '</b>';
