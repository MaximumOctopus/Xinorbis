...
void TabUiLength::Chart(TChart* chart, int DataSource, int label_options)
{
	...
	if (GScanEngine->Data[DataSource].Path.LastScanMultiple)
	{
	...
	for (int t = 0; t < 280; t++)
	{
		if (chart->Tag == 1)
		{
			if (GScanEngine->Data[DataSource].Lengths[t].Count != 0)
			{
				std::wstring label = L"";
				...
				case kLabelOptionDefault:
					label = GScanEngine->Data[DataSource].Lengths[t].Name;
					break;
				case kLabelOptionMostConvenient:
					label = Convert::ConvertToUsefulUnit(GScanEngine->Data[DataSource].Lengths[t].Size);
					break;
				case kLabelOptionKilobyte:
					label = Convert::ConvertToStaticUnit(GScanEngine->Data[DataSource].Lengths[t].Size, UnitConversion::kUnitKB);
					break;
				case kLabelOptionMegabyte:
					label = Convert::ConvertToStaticUnit(GScanEngine->Data[DataSource].Lengths[t].Size, UnitConversion::kUnitMB);
					break;
				case kLabelOptionGigabyte:
					label = Convert::ConvertToStaticUnit(GScanEngine->Data[DataSource].Lengths[t].Size, UnitConversion::kUnitGB);
					break;
				}

				chart->SeriesList->Items[0]->Add(GScanEngine->Data[DataSource].Lengths[t].Count, label.c_str(), TColor(kSpectrumColours[t % kSpectrumMod]));
			}
		}
		else
		{
            if (GScanEngine->Data[DataSource].Lengths[t].Size != 0)
			{
				std::wstring label = L"";
				...
				chart->SeriesList->Items[0]->Add(GScanEngine->Data[DataSource].Lengths[t].Size, label.c_str(), TColor(kSpectrumColours[t % kSpectrumMod]));
			}
		}
	}
}
...
void TabUiLength::Table(TStringGrid* grid, int DataSource, int index, bool add_all)
{
