...
void TabUiNull::TableFiles(TStringGrid* grid, int DataSource)
{
...
//	oTable.ClearRows(1, oTable.RowCount - 1);
	...
	for (FileObject *file : GScanEngine->Data[DataSource].Files)
	{
		if (!(faDirectory & file->Attributes))
		{
			if (file->Size == 0)
			{
				grid->Cells[0][count + 1] = (GScanEngine->Data[DataSource].Folders[file->FilePathIndex] + file->Name).c_str();
...
void TabUiNull::TableFolders(TStringGrid* grid, int DataSource)
{
}
