...
void TabUiTypes::Tree(TTreeView* view, int DataSource, int index)
{
	view->Items->Clear();
	...
	for (int t = 0; t < kFileCategoriesCount; t++)
	{
		...
		if (AddRow)
		{
			std::wstring numstr = GScanEngine->Data[DataSource].ExtensionSpread[t].PercentCountString;
			std::wstring ftsstr = GScanEngine->Data[DataSource].ExtensionSpread[t].PercentSizeString;

			std::wstring cx = GScanEngine->Data[DataSource].ExtensionSpread[t].Name + L" [ " +
							  std::to_wstring(GScanEngine->Data[DataSource].ExtensionSpread[t].Count) + L" " + GLanguageHandler->Text[kFiles] +
							  L" (" + numstr + L") / " + Convert::ConvertToUsefulUnit(GScanEngine->Data[DataSource].ExtensionSpread[t].Size) + L" (" + ftsstr + L") ]";
...
void TabUiTypes::List(TStringGrid* grid, int DataSource)
{
...
void TabUiTypes::Chart(TChart* chart, int DataSource, int label_options)
{
		...
		if (tfx->Category == GSettingsHandler->TabInternal.TreeViewChartIndex)
		{
			...
			case kSelectionQuantity:
				if (tfx->Count != 0)
				{
					...
					chart->SeriesList->Items[0]->Add(tfx->Count, label.c_str(), TColor(kSpectrumColours[ci % kSpectrumMod]));
				}
				break;
			case kSelectionSize:
				if (tfx->Size != 0)
				{
					...
					chart->SeriesList->Items[0]->Add(tfx->Size, label.c_str(), TColor(kSpectrumColours[ci % kSpectrumMod]));
				}
				break;
			}
