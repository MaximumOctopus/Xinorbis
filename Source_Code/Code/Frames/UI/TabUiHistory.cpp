...
int TabUiHistory::AddBar(TChart* chart, std::vector<std::wstring> &search_strings, TDateTime dtcurrent_date, int interval, bool quantity, int count, unsigned __int64 size, int current_date, int colour_mod)
{
	...
	if (quantity)
	{
		chart->SeriesList->Items[0]->Add(count, title.c_str(), TColor(kSpectrumColours[colour_mod % kSpectrumMod]));
	}
	else
	{
		chart->SeriesList->Items[0]->Add(size, title.c_str(), TColor(kSpectrumColours[colour_mod % kSpectrumMod]));
	}
...
std::wstring TabUiHistory::Chart(TChart* chart, std::vector<std::wstring> &search_strings, int DataSource, int user_id,
								 ...
								 std::wstring interval_caption)
{
	...
	unsigned __int64 ExtSpreadDates[kFileCategoriesCount][2];
	...
	if (interval <= kIntervalYear)
	{
		...
		{
			FileObject *xfo = GScanEngine->Data[DataSource].Files[f];
			...
			if (file_date >= FromDate && file_date <= ToDate)
			{
				if (!(faDirectory & xfo->Attributes))
				{
					...
					{
					case kIntervalDay:
						if (f == GScanEngine->Data[DataSource].Files.size() - 1)
						{
							ColourMod = AddBar(chart, search_strings, dt_current_date, interval, quantity, count, size, current_date, ColourMod);
